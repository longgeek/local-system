...
c.replace(m,d).replace(n,f).replace(o,g).replace(p,h);else if(e.direction==="up"&&(b<=a||e.minus))c=c.replace(m,d).replace(n,f).replace(o,g).replace(p,h);else{c=c.replace(t," ");e.hasCompleted=true}this.html(c);this.trigger("change",[e]);if(e.hasCompleted){this.trigger("complete.jcountdown");window.clearInterval(e.timer)}}},w={init:function(c){var a=k.extend({},x,c),b;b=a.htmlTemplate;return this.each(function(){var f=k(this),d={},h=a.offset===null?new Date:u(a.offset),g=new Date(a.date),e=(a.direction===
